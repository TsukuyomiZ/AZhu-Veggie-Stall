...
| 表單 | `.field` `.field-label` `.input` `.select` `.textarea` `.field-hint` `.field-error` `.input-error` `.form-row` `.item-card` `.item-row`(+ `.item-row-name` `.item-row-qty` `.item-row-price`)`.chip-reveal`(+ `.chip-reveal-open`)`.unit-chips` `.unit-chip`(+ `.unit-chip-active`)`.total-bar` `.total-bar-label` |
